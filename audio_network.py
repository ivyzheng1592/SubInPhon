...
class AudDecoder(nn.Module):
    def __init__(self, input_dim, embedding_dim, hidden_dim, output_dim, n_layers, dropout, attention):
        super(AudDecoder, self).__init__()
...
class AudioSeq2Seq(nn.Module):
    def __init__(self, encoder_input_dim, decoder_input_dim, encoder_embedding_dim, decoder_embedding_dim,
        ...
        self.txt_decoder = TxtDecoder(decoder_input_dim, decoder_embedding_dim, hidden_dim, output_dim,
                                      n_layers, decoder_dropout, self.attention).to(self.device)
        self.aud_decoder = AudDecoder(decoder_input_dim, decoder_embedding_dim, hidden_dim, output_dim,
                                      n_layers, decoder_dropout, self.attention).to(self.device)

    def forward(self, src, trg, teacher_forcing_ratio=0.5):
        # src = ([src_len, batch_size], [batch_size, n_channels, freq, src_len])
        # trg = ([trg_len, batch_size], [batch_size, n_channels, freq, trg_len])
        ...
        txt_decoder_outputs = torch.zeros(txt_trg_len, batch_size, self.output_dim).to(self.device)
        txt_predictions = torch.zeros(txt_trg_len, batch_size).to(self.device)
        # decoder_outputs store the probability of all output vocabulary for each input text token
        # predictions store the predicted output token for each input text token
        ...
        for t in range(1, txt_trg_len):
            ...
            txt_output, hidden, cell, _ = self.txt_decoder(txt_input, encoder_states, hidden, cell)
            # output = [batch_size, output_dim]
            ...
            txt_decoder_outputs[t] = txt_output
            txt_predictions[t] = best_guess
        ...
        return txt_decoder_outputs, txt_predictions
