...
if __name__ == "__main__":
    ...
    parser.add_argument('--timestamp', '-ts', type=str, default="0000000000",
                        help="Timestamp for project, better be generated by bash")
    parser.add_argument('--gpu', '-gpu', type=int, default=0, help="Choose the GPU to work on")
    parser.add_argument('--prenet_dim', '-p', type=int, default=32, help="Prenet dimension")
    parser.add_argument('--audio_hidden_dim', '-h', type=int, default=8, help="Hidden dimension")
    parser.add_argument('--learning_rate', '-l', type=int, default=1e-3, help="Learning rate")

    args = parser.parse_args()
    hp.prenet_dim = args.prenet_dim
    hp.audio_hidden_dim = args.audio_hidden_dim
    hp.learning_rate = args.learning_rate

    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print(f"Using {device} device")

    trial_num = "2511202120"  # time stamp
    #trial_num = args.timestamp
