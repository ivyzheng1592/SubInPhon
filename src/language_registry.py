...
class LanguageRegistry:
    ...
    def build_one(self, name: str, variant: Optional[str] = None) -> Any:
        ...
        return self._build_language(name, spec, variant=variant)

    def build(self, variant: Optional[str] = None) -> Dict[str, Any]:
        languages = {name: self._build_language(name, spec, variant=variant) for name, spec in self._specs.items()}
        return languages

    def _build_onset(self, onset_spec: Any) -> Dict[str, Any]:
    ...
    def _build_coda(self, coda_spec: Any) -> Dict[str, Any]:
    ...
    def _build_vowel(self, vowel_spec: Any) -> Dict[str, Any]:
    ...
    def _template_to_bits(self, template: str) -> str:
    ...
    def _convert_syll_struct(self, syll_struct: Dict[str, Any]) -> Dict[str, Any]:
        return {self._template_to_bits(k): v for k, v in syll_struct.items()}

    def _convert_word_struct(self, word_struct: List[str]) -> List[str]:
        converted = []
        for struct in word_struct:
            parts = struct.split("-")
            converted.append("-".join(self._template_to_bits(p) for p in parts))
        return converted

    def _build_language(self, name: str, spec: Dict[str, Any], variant: Optional[str] = None) -> Any:
        onset = self._build_onset(spec["onset"])
        coda = self._build_coda(spec["coda"])
        vowel = self._build_vowel(spec["vowel"])
        syll_struct = self._convert_syll_struct(spec["syll_struct"])
        word_struct = self._convert_word_struct(spec["word_struct"])
...
# language = registry.build_one("EnglishBH_shortened")
